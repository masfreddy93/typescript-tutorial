...
let myObj2: {a: string, b: number, c?: boolean}  //N.B. il punto di domanda dopo c significa che posso anche omettere c in fase di assegnazione di un oggetto! se non metto il punto di domanda devo per forza assegnare un valore a c ad ogni assegnazione 
myObj2 = {
    a: 'ehi',
    b: 5,
}
...
let myFunc: () => void = () => {  //se la funzione non ritorna nulla si usa il 'void' altrimenti si mette il tipo di ritorno! (es. string, number ecc)
    //codice vario
} 

//dichiare funzione con la keyword 'function' 
function myFunc2(param1: string, param2: string): string{
...
//ANY & UNKNOWN
//any
let gen: any  //dichiarando una variabile di tipo any POTRò assegnarrgli qualsiasi tipo di valore!
gen = 'stringa'
gen = 5
gen = false
gen = []
...
function myFunc3(param: any): void{
    param.saluta()
}
function myFunc4(param: unknown): void{
    param.saluta()
}
//che succede? succede che se metto il param any la funzione mi accetta un codice con il metodo 'saluta' del parametro any ('potrebbe anche avrcelo quel metodo'). Con unknown invece NO!
//in breve, unknown è più RESTRITTIVO!
//fonte: https://www.youtube.com/watch?v=x1zWm_PeXgU&list=PLYrQFCVhfFIt4k21sOv8Ll29ZxLQTSmR1&index=4



//TYPE, INTERFACE, ENUM
//tra type e interface le differenze sono davvero poche
//si usano quando abbiamo a che fare con oggetti un po' più complessi di questo---> let myObj: {a: string, b: number, c: boolean}

//type
type newType = { //dichiarazione del tipo
    a: string,
    b: number,
    c?: boolean
}
let myTipe: newType = {  //dichiarazione della variabile myTipe di tipo newType! ovviamente dovrà avere attributi 'a' e 'b' mentre attributo 'c' è facoltativo
    a: 'ciao',
    b: 2,
}

//interface
//per prassi hanno sempre la 'I' maiuscola all'inizio del nome e non hanno i ':'
interface INewInter {
    a: string,
    b: number,
    c: boolean,
    ...
    e: number[],
    f: () => string,
    g: {p1: string, p2: number}
    [key: string]: any  //questo mi permette di inserire altri attributi nelle variabili generate da questa interfaccia!
}
let myInter: INewInter = {  //dich e assegnaz di variabile myInter
    a: 'ciao',
    b: 2,
    c: false,
    e: [2, 4],
    f: () => 'ciao',
    g: {p1: 'ciao', p2: 6},
    h: 5,  //n.b. questo non da errore grazie a 'key: string' nell'interfaccia
...
let mas: Persona = new Persona('Nicola', 29)
console.log(mas)
...
let mamma: Genitore = new Genitore('Monica', 56)
mamma.saluta()
